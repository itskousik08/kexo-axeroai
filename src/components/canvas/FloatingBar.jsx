...
const BAR_ITEMS = [
  { id: 'note',     label: 'Note',     icon: icons.note,     action: 'add',          color: 'var(--green)' },
  { id: 'concept',  label: 'Concept',  icon: icons.concept,  action: 'add',          color: 'var(--accent)' },
  { id: 'question', label: 'Question', icon: icons.question, action: 'add',          color: 'var(--violet)' },
  { id: 'image',    label: 'Image',    icon: icons.image,    action: 'upload-image', color: 'var(--blue)' },
  { id: 'code',     label: 'Code',     icon: icons.code,     action: 'add',          color: 'var(--rose)' },
  { id: 'link',     label: 'Link',     icon: icons.link,     action: 'add',          color: 'var(--blue)' },
  { id: 'unlink',   label: 'Unlink',   icon: icons.unlink,   action: 'unlink',       color: 'var(--text2)' },
  { id: 'voice',    label: 'Voice',    icon: icons.voice,    action: 'voice',        color: 'var(--violet)' },
  { id: 'ai',       label: 'AI',       icon: icons.ai,       action: 'ai',           color: 'var(--accent)' },
];
...
export default function FloatingBar({ hidden }) {
  ...
  const handleItem = (item) => {
    ...
    switch (item.action) {
      case 'add': actions.addNode(item.id); break;
      case 'upload-image': imageInputRef.current?.click(); break;
      case 'unlink':
        if (state.selectedNode) actions.enterUnlinkMode(state.selectedNode);
        else actions.showToast('Select a node first');
        break;
      case 'voice': actions.toggleVoiceModal(); break;
      case 'ai': actions.toggleAIPanel(); break;
    }
  };
    ...
    <>
      <div className="floating-bar">
        <div className="floating-bar__track">
          {BAR_ITEMS.map(item => {
            const isOn = (item.id === 'ai' && state.aiPanelOpen) || (item.id === 'unlink' && state.unlinkFirstNode);
            return (
              <button
                key={item.id}
                className={'fb-btn' + (activeId === item.id ? ' fb-btn--active' : '') + (isOn ? ' fb-btn--on' : '')}
