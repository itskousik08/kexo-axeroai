...
export default function TraditionalCanvas() {
  ...
  const [pendingImageAfter, setPendingImageAfter] = useState(null);

  // Load saved data
  useEffect(() => {
    if (state.notes) {
      try {
        const b = JSON.parse(state.notes);
  ...
  const handleImageFile = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      setBlocks(prev => {
        if (!pendingImageAfter) return [...prev, newBlock];
        const idx = prev.findIndex(b => b.id === pendingImageAfter);
        const next = [...prev];
        next.splice(idx + 1, 0, newBlock);
        return next;
      });
    ...
    e.target.value = '';
    setPendingImageAfter(null);
  };

  // Voice modal adds a voice block via FloatingBar
  // We intercept the global voice add event
  useEffect(() => {
    const handler = (e) => {
      const { audioData, name } = e.detail || {};
      if (!audioData) return;
      const newBlock = { id: ++nextIdRef.current, type: 'audio', content: audioData, caption: name || 'Voice Note' };
      setBlocks(prev => [...prev, newBlock]);
    };
    window.addEventListener('kexo:tradVoiceAdd', handler);
    return () => window.removeEventListener('kexo:tradVoiceAdd', handler);
  }, []);
          ...
          {blocks.map((block, idx) => (
            <div key={block.id} className="trad-block">
              ...
              {block.type === 'image' && (
                <div className="trad-block__image">
                  ...
                  <div
                    ...
                    contentEditable suppressContentEditableWarning
                    onInput={e => updateBlock(block.id, { caption: e.currentTarget.innerText })}
                  >{block.caption}</div>
                </div>
              )}
              ...
              <div className="trad-block__actions">
                <label
                  className="trad-act-btn"
                  title="Add image"
                  onClick={() => setPendingImageAfter(block.id)}
                >
